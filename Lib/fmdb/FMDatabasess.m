...
@interface FMDatabasess ()

- (FMResultSets *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;

@end

@implementation FMDatabasess
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    FMDatabasess *self = (__bridge FMDatabasess*)f;
...
- (void)closeOpenResultSets {
    ...
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        FMResultSets *rs = (FMResultSets *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(FMResultSets *)resultSet {
...
- (void)clearCachedStatements {
    ...
    for (NSMutableSet *statements in [_cachedStatements objectEnumerator]) {
        for (FMStatements *statement in [statements allObjects]) {
            [statement close];
        }
    }
...
- (FMStatements*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(FMStatements* statement, BOOL *stop) {
...
- (void)setCachedStatement:(FMStatements*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
    ...
    FMResultSets *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (FMResultSets *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
...
- (FMResultSets *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    FMStatements *statement  = 0x00;
    FMResultSets *rs         = 0x00;
    ...
    if (!statement) {
        statement = [[FMStatements alloc] init];
    ...
    rs = [FMResultSets resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (FMResultSets *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (FMResultSets *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (FMResultSets *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (FMResultSets *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error {
    FMResultSets *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil];
    if (!rs && error) {
...
- (FMResultSets *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[FMStatements alloc] init];
...
void FMDBBlockSQLiteCallBackFunctions(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void FMDBBlockSQLiteCallBackFunctions(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &FMDBBlockSQLiteCallBackFunctions, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &FMDBBlockSQLiteCallBackFunctions, 0x00, 0x00);
#endif
}
