...
@interface singleMicroViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout>
...
@property(nonatomic,weak) microCollectionViewCell *cell;
...
static NSString *string = @"microCollectionViewCell";

static NSInteger   indexNum;

static char  arry[2] = {'0','0'};

@implementation singleMicroViewController
    ...
    [socket  receiveMsg:^(NSString *receiveInfo) {
        ...
        switch (deviceType_ID) {
            ...
            case 51:
                ...
                if ([data_type  isEqualToString:@"0c00"]) {
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    ...
    if ([_dataCenter.gatewayNo isEqualToString:@"0"] | !_dataCenter.gatewayNo ) {
        ...
        count =[deviceMessageTool queryWithSensorsDevices:devicess].count;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    _cell.button.hidden = NO;
    _cell.button.selected = NO;
    ...
    switch (device.DEVICE_TYPE_ID  ) {
        ...
        case 115:
            ...
            if ([[device.DEVICE_STATE  substringToIndex:2] isEqualToString:@"10"]|[[device.DEVICE_STATE  substringToIndex:2] isEqualToString:@"00"]) {//撤防
...
- (void)enterChangeButton:(UIButton *)btn {
    ...
    microCollectionViewCell *cell = (microCollectionViewCell *)[[[btn superview]superview]superview];//获取cell
...
-(void)switchOnAction {
    ...
    NSString *dev_type = [[ControlMethods  ToHex:dev.DEVICE_TYPE_ID]  stringByAppendingString:@"00"];
    ...
    if (_dataCenter.networkStateFlag == 0) {//内网
        ...
        NSString *localControlMessge = [deviceControlPacketStr  stringByReplacingCharactersInRange:NSMakeRange(0, 8) withString:@"41414444"];
        //打印报文
        NSLog(@"==内网localControlMessge is ===== %@==",localControlMessge);
        [socket sendMsg:localControlMessge];
...
-(void)addUICollectionView{
    ...
    [collectionView registerClass:[microCollectionViewCell class] forCellWithReuseIdentifier:string];
...
-(void)setNavBar{
    ...
    //创建一个导航栏集合
    UINavigationItem *navItem = [[UINavigationItem alloc] initWithTitle:@"微控"];
