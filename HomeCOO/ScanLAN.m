...
- (void)startScan {
   ...
   // NSLog(@"==self.localAddress = %@====",self.localAddress);
    ...
    //self.localAddress = @"192.168.20.1";
...
- (void)pingAddress{
  ...
  //  NSLog(@"ping  IP %ld",self.currentHostAddress);
...
- (void)pingResult:(NSNumber*)success {
    ...
    // 将 任务 添加到 全局并发队列中 异步执行 具备开启新的线程的能力
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (success.boolValue) {
           ...
           // NSLog(@"SUCCESS");
            ...
            NSString *deviceIPAddress = [[[[NSString stringWithFormat:@"%@%ld", self.baseAddress, (long)
                                            self.currentHostAddress] stringByReplacingOccurrencesOfString:@".0" withString:@"."] stringByReplacingOccurrencesOfString:@".00" withString:@"."] stringByReplacingOccurrencesOfString:@".." withString:@".0."];
