...
-(void)layoutChildViews{
    ...
    //添加设备详细信息标签
    self.spaceMessageLable = [[UILabel  alloc]initWithFrame:CGRectMake(0, 0, oneSwitchW/1.3, 40)];
    
    self.spaceMessageLable.clipsToBounds = YES;
    self.spaceMessageLable.layer.cornerRadius = 8.0;//设置边框圆角
    self.spaceMessageLable.text = @"";//默认位置待定
    self.spaceMessageLable.textAlignment = NSTextAlignmentCenter;
    self.spaceMessageLable.font =[UIFont  systemFontOfSize:12];
    //self.spaceMessageLable.backgroundColor = [UIColor  blueColor];
    [self.imgView   addSubview:self.spaceMessageLable];
