...
static scanerSocketManger *socketManager = nil;

@interface scanerSocketManger ()<GCDAsyncSocketDelegate>
{
    NSString *_host;
    uint16_t _port;
...
@implementation scanerSocketManger
...
+ (instancetype)shareSocketManager{
    ...
    dispatch_once(&onceToken, ^{
        ...
        socketManager = [[scanerSocketManger alloc]init];
...
-(void)closeSocket{
    ...
    self.connectStatus =  SSGACSocketConnectStatusDisconnected;
...
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err{
    ...
    if (self.connectStatus == SSGACSocketConnectStatusDisconnected) {//主动断开
