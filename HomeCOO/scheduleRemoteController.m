...
@interface scheduleRemoteController ()<UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,HCDaterViewDelegate,scheduleInfraDataViewDelegate>
...
@property(nonatomic,strong) scheduleInfraDataView *scheduleView;
...
@implementation scheduleRemoteController
...
-(void)queryInfraTimingSchduleTasks{
    ...
    if(self.scheduleListArray.count!=0){
        ...
        for (int i = 0; i<self.scheduleListArray.count; i++) {
            ...
            if ([schedules.deviceNo isEqualToString:_dataCenter.remoteIEEAddress]) {//说明是定时遥控器任务
...
- (scheduleInfraDataView *)scheduleView{
    
    // if(_scheduleView == nil){
    
    _scheduleView = [[scheduleInfraDataView alloc]initWithFrame:CGRectZero];
...
#pragma mark -daterViewClicked delegate
...
#pragma mark -makeSureClicked infraClicked delagate
    ...
    switch (daterView.currentTag) {
        case 1://单次
            dict = @{@"scheduleId":self.scheduleID,
                     ...
                     @"deviceNo":_dataCenter.remoteIEEAddress,
                     @"deviceState": [daterView.packData substringFromIndex:60] ,
                     @"scheduleName": daterView.scheduleName ,
