...
- (void)selectCode
{
    ...
    UIAlertAction *action2 = [UIAlertAction actionWithTitle:@"智能匹配" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [LZXDataCenter defaultDataCenter].intellectMatchFlag = 1;
...
- (void)IRCodeDidMatched:(NSNotification *)noti
{
    ...
    self.smm.ctrBtnClicked = ^(NSInteger tag){
        if(tag==0)//save
        {
            ...
            deviceName =[NSString stringWithFormat:@"%@品牌%ld",device.brandCN,(long)device.type];
            ...
            if ([LZXDataCenter defaultDataCenter].intellectMatchFlag==1) {
                
                [CYdeviceMessageTool addDevice:cyDevice];
                
                [LZXDataCenter defaultDataCenter].intellectMatchFlag=0;
            }
            
            
            [wself.navigationController  pushViewController:[[CYDevicesVC alloc]init] animated:YES];
