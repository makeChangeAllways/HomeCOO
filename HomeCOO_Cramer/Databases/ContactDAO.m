...
-(BOOL)insert:(Contact *)contact{
    ...
    BOOL result = NO;
    if([self openDB]){
        NSString *SQL = [NSString stringWithFormat:@"INSERT INTO Contact(activeUser,contactId,contactName,contactPassword,contactType,messageCount) VALUES(\"%@\",\"%@\",\"%@\",\"%@\",\"%i\",\"%li\")",loginResult.contactId,contact.contactId,contact.contactName,contact.contactPassword,contact.contactType,(long)contact.messageCount];
...
-(Contact*)isContact:(NSString *)contactId{
    Contact *contact = nil;
    NSMutableArray *array = [NSMutableArray arrayWithCapacity:0];
    if(![UDManager isLogin]){//在主工程里面已经登录过了
        return nil;
    }
...
-(BOOL)update:(Contact *)contact{
    ...
    BOOL result = YES;
    if([self openDB]){
        NSString *SQL = [NSString stringWithFormat:@"UPDATE Contact SET CONTACTID = \"%@\", CONTACTNAME = \"%@\",CONTACTPASSWORD = \"%@\",CONTACTTYPE = \"%li\",MESSAGECOUNT = \"%i\" WHERE ACTIVEUSER = \"%@\" AND CONTACTID = \"%@\"",contact.contactId,contact.contactName,contact.contactPassword,(long)contact.contactType,contact.messageCount,loginResult.contactId,contact.contactId];
        DLog(@"%@",SQL);
        if(sqlite3_prepare_v2(self.db, [SQL UTF8String], -1, &statement, NULL)!=SQLITE_OK){
