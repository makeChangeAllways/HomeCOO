...
-(void)initComponent{
    ...
    [topBar setTitle:NSLocalizedString(@"discover",nil)];
    ...
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, NAVIGATION_BAR_HEIGHT+ANIM_VIEW_WIDTH_AND_HEIGHT, width, height-ANIM_VIEW_WIDTH_AND_HEIGHT-NAVIGATION_BAR_HEIGHT) style:UITableViewStyleGrouped];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    @synchronized(self){
        ...
        if(contact!=nil&&flag==1){
            [cell setRightIcon:@"ic_shake_monitor.png"];
        }else{
            [cell setRightIcon:@"ic_shake_add.png"];
        }
...
//        
//        if ([localDevic.contactId isEqualToString:contactId]) {
//            CreateInitPasswordController * createInitPwdCtl = [[CreateInitPasswordController alloc] init];
//            createInitPwdCtl.contactId = contactId;
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [UIView transitionWithView:self.toastView duration:0.2 options:UIViewAnimationCurveEaseInOut
                                ...
                                completion:^(BOOL finished) {
                                    [self.toastView setHidden:YES];
                                    if ([[self.types allKeys] count] == 0) {
                                        [self.view makeToast:NSLocalizedString(@"no_device", nil)];
                                    }
                                }
                 ];
            });
