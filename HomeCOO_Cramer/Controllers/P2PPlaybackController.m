        ...
        case RET_GET_PLAYBACK_FILES:
        {
            ...
            if (self.selectedLabel==1) {
                for (NSString *file in array){
                    DLog(@"%@",file);
                    if ([self.dayOneFiles containsObject:file]) {
                        DLog(@"Repeat object");
                ...
                for (NSString *file in array){
                    DLog(@"%@",file);
                    if ([self.dayThreeFiles containsObject:file]) {
                        DLog(@"Repeat object");
                ...
                for (NSString *file in array){
                    DLog(@"%@",file);
                    if ([self.dayMonthFiles containsObject:file]) {
                        DLog(@"Repeat object");
                ...
                for (NSString *file in array){
                    DLog(@"%@",file);
                    if ([self.customFiles containsObject:file]) {
                        DLog(@"Repeat object");
            ...
            NSLog(@"%lu",(unsigned long)[self.playbackFiles count]);
...
-(void)initComponent{
    ...
    [tableView addInfiniteScrollingWithActionHandler:^{
        ...
        NSDate * startDate = [Utils dateFromString:self.nextStartTime];
        
        if (self.selectedLabel==1) {
            NSString * endTime2 = [NSString stringWithFormat:@"%i-%i-%i %i:%i",year,month,day-1,hour,minute];
            NSDate * endDate = [Utils dateFromString:endTime2];
            [[P2PClient sharedClient] getPlaybackFilesWithIdByDate:self.contact.contactId password:self.contact.contactPassword startDate:endDate endDate:startDate];
        }else if (self.selectedLabel==2){
            NSString * endTime2 = [NSString stringWithFormat:@"%i-%i-%i %i:%i",year,month,day-3,hour,minute];
            NSDate * endDate = [Utils dateFromString:endTime2];
            [[P2PClient sharedClient] getPlaybackFilesWithIdByDate:self.contact.contactId password:self.contact.contactPassword startDate:endDate endDate:startDate];
        }else if (self.selectedLabel==3){
            NSString * endTime2 = [NSString stringWithFormat:@"%i-%i-%i %i:%i",year,month-1,31-day,hour,minute];
            NSDate * endDate = [Utils dateFromString:endTime2];
            [[P2PClient sharedClient] getPlaybackFilesWithIdByDate:self.contact.contactId password:self.contact.contactPassword startDate:endDate endDate:startDate];
        }else if (self.selectedLabel==4){
            NSDate *customStartDate = [Utils dateFromString:self.startTime];
            [[P2PClient sharedClient] getPlaybackFilesWithIdByDate:self.contact.contactId password:self.contact.contactPassword startDate:customStartDate endDate:startDate];
        }
...
-(void)onCustomSearch:(UIButton*)button{
    ...
    [UIView transitionWithView:self.searchMaskView duration:0.3 options:UIViewAnimationCurveEaseOut animations:^{
    ...
    if(self.isShowCustomView){
        ...
        [UIView transitionWithView:self.customView duration:0.2 options:UIViewAnimationCurveEaseOut
                        animations:^{
...
-(void)onButtonPress:(id)sender{
    ...
    [UIView transitionWithView:self.layerView duration:0.2 options:UIViewAnimationCurveEaseOut
                    animations:^{
