...
-(void)layoutSubviews{
    ...
    if(!self.rightLabelView){
        ...
        self.rightLabelView.userInteractionEnabled = YES;
        [textLabel release];
    ...
    if (!self.tap) {
        ...
        [self.rightLabelView addGestureRecognizer:tap];
        tap.numberOfTapsRequired = 1;
        self.tag = tap;
    }else
    {
        [self.rightLabelView addGestureRecognizer:self.tap];
        self.tap.numberOfTapsRequired = 1;
    }
    DLog(@"%f %f",cellWidth,cellHeight);
    if(!self.customView){
        
        TimezoneView *timezoneView = [[TimezoneView alloc] initWithFrame:CGRectMake(30,5, cellWidth-30*2, cellHeight-5*2) value:self.value];
