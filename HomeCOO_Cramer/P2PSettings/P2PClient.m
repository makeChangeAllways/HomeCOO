...
//    mP2Pprm.pHostName = "|www.cloudlinks.cn|www.cloud-links.net|www.gwelltimes.com|www.2cu.co";
    ...
    mP2Pprm.vFlagUpdate = vFlagUpdate;//没用到则 = NULL
...
//    }
    ...
    if(pdwFlag[1] > dwMyFlag[1] || dwMyFlag[1] - pdwFlag[1] > 32767)
    {
...
#define MESG_ID_GET_ALARM_INFO 45000
...
void  vSendMessageAck(DWORD dwDesID,DWORD dwMesgID, DWORD  dwError)
{
    ...
    if(dwMesgID<MESG_ID_GET_PLAYBACK_FILES&&dwMesgID>=(MESG_ID_GET_PLAYBACK_FILES-1000)){
        ...
        [[P2PClient sharedClient] ack_receiveReceiveRemoteMessage:parameter];
    }else if(dwMesgID<MESG_ID_GET_ALARM_INFO&&dwMesgID>=(MESG_ID_GET_ALARM_INFO-1000)){
        NSNumber *key = [NSNumber numberWithInt:ACK_RET_GET_ALARM_INFO];
...
void  vRecvRemoteMessage(DWORD dwSrcID,  unsigned int fgHasCheckdPassword, void *pMesg, DWORD dwMesgSize)
{
    ...
    if(pMesgBuffer[0]==MESG_TYPE_RET_REC_LIST){
        ...
        }else
        {
            ...
            if ([contact_more isEqualToString:@"contact"]) {
//                playbackFiles = [];
...
//                NSLog(@"%@",playbackFiles);
        ...
        [[P2PClient sharedClient] receiveReceiveRemoteMessage:parameter];
    }else if(pMesgBuffer[0] == MESG_TYPE_RET_ALARM_INFO){
        sMesgAlarmInfo * sAlarmInfo = (sMesgAlarmInfo *)pMesg;
        
        NSMutableDictionary *parameter = [NSMutableDictionary dictionaryWithCapacity:0];
        NSNumber *key = [NSNumber numberWithInt:RET_GET_ALARM_INFO];
        [parameter setValue:key forKey:@"key"];
        
        NSString * result = [NSString stringWithFormat:@"%d",sAlarmInfo->bOption];
        [parameter setValue:result forKey:@"result"];
        
        
        if (sAlarmInfo->bOption == MESG_GET_OK) {//表示获取成功
            //报警记录的个数
            NSString * bAlarmRecordNumber = [NSString stringWithFormat:@"%d",sAlarmInfo->bAlarmInfoNs];
            
            //最后一个报警记录编号
            NSString * dwLastAlarmInfoIndex = [NSString stringWithFormat:@"%d",sAlarmInfo->dwLastAlarmInfoIndex];
            
            //设备ID
            NSString *deviceId = [NSString stringWithFormat:@"%i",dwSrcID];
            
            //取出本地的记录
            AlarmDAO * alarmDAO = [[AlarmDAO alloc] init];
            NSArray *arr = [alarmDAO findAllUnsortedWithDeviceID:deviceId];
            
            for (NSInteger i = 0; i < bAlarmRecordNumber.integerValue; i++) {
                BOOL isEqual = NO;
                
                NSString * dwTime_s = [NSString stringWithFormat:@"%d",sAlarmInfo->bAlarmInfo[i].dwTime_s];
                
                NSString * bAlarmType = [NSString stringWithFormat:@"%d",sAlarmInfo->bAlarmInfo[i].bAlarmType];
                NSString * bDefenceAndChannel = [NSString stringWithFormat:@"%02x",sAlarmInfo->bAlarmInfo[i].bDefenceAndChannel];
                NSString *defence = [NSString stringWithFormat:@"%c",[bDefenceAndChannel characterAtIndex:0]];
                NSString *channel = [NSString stringWithFormat:@"%c",[bDefenceAndChannel characterAtIndex:1]];
                
                for(NSInteger j = 0; j < arr.count; j++){
                    BOOL isEqual1, isEqual2, isEqual3, isEqual4;
//                    if (i == j) {//匹配index
//                        isEqual1 = YES;
//                    }else{
//                        isEqual1 = NO;
//                    }
                    ...
                    Alarm *alarm = arr[j];
                    if ([dwTime_s isEqualToString:alarm.alarmTime]) {//匹配dwTime_s
                        isEqual1 = YES;
                    }else{
                        isEqual1 = NO;
                    }
                    
                    if ([bAlarmType isEqualToString:[NSString stringWithFormat:@"%d",alarm.alarmType]]) {//匹配bAlarmType
                        isEqual2 = YES;
                    }else{
                        isEqual2 = NO;
                    }
                    
                    if ([defence isEqualToString:[NSString stringWithFormat:@"%d",alarm.alarmGroup]]) {//匹配defence
                        isEqual3 = YES;
                    }else{
                        isEqual3 = NO;
                    }
                    
                    if ([channel isEqualToString:[NSString stringWithFormat:@"%d",alarm.alarmItem]]) {//匹配channel
                        isEqual4 = YES;
                    }else{
                        isEqual4 = NO;
                    }
                    
                    if(isEqual1&&isEqual2&&isEqual3&&isEqual4){//YES表示此条记录已插入
                        isEqual = YES;
                        break;
                    }
                }
                ...
                if (!isEqual) {//此条记录在latestAlarm之后，不存在本地，插入
                    Alarm * alarm = [[Alarm alloc] init];
                    
                    alarm.deviceId = deviceId;
                    alarm.alarmTime = dwTime_s;
                    alarm.alarmType = bAlarmType.intValue;
                    alarm.alarmGroup = defence.intValue;
                    alarm.alarmItem = channel.intValue;
                    
                    NSString * plist = [[NSBundle mainBundle] pathForResource:@"Common-Configuration" ofType:@"plist"];
                    NSDictionary * dic = [NSDictionary dictionaryWithContentsOfFile:plist];
                    BOOL isDeviceAlarmRecord = [dic[@"isDeviceAlarmRecord"] boolValue];
                    if (isDeviceAlarmRecord) {//YES则保存到本地
                        [alarmDAO insert:alarm];
                    }
                    [alarm release];
                }else{//此条记录已插入
...
#pragma mark - 获取报警记录信息(app->设备)
-(void)getAlarmInfoWithId:(NSString *)contactId password:(NSString *)password
{
    static int mesg_id = MESG_ID_GET_ALARM_INFO - 1000;
    if (mesg_id >= MESG_ID_GET_ALARM_INFO) {
        mesg_id = MESG_ID_GET_ALARM_INFO - 1000;
    }
    
    sMesgAlarmInfo sAlarmInfo;
    sAlarmInfo.bCmd = MESG_TYPE_GET_ALARM_INFO;
    sAlarmInfo.bOption = 0;
    sAlarmInfo.bAlarmInfoNs = 0;
    sAlarmInfo.bReseve = 0;
    sAlarmInfo.dwLastAlarmInfoIndex = 0;
    fgP2PSendRemoteMessage(contactId.intValue, password.intValue, mesg_id, &sAlarmInfo, sizeof(sMesgAlarmInfo), NULL, 0, 0);
    mesg_id++;
}
...
-(void)getAddPlaybackFilesWithIdByDate:(NSString*)contactId password:(NSString*)password startDate:(NSDate*)startDate endDate:(NSDate*)endDate type:(NSString *)type{
