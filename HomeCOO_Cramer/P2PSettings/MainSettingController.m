    ...
    switch(key){
        ...
        case RET_DO_DEVICE_UPDATE:
        {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if(result==1){
                    self.progressLabel.text = [NSString stringWithFormat:@"%li%%",(long)value];
...
//    [topInfoBarView release];
    ...
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0,TOP_INFO_BAR_HEIGHT-70, width, height-(NAVIGATION_BAR_HEIGHT+TOP_INFO_BAR_HEIGHT)+70) style:UITableViewStyleGrouped];
    ...
    switch(section){
        case 0:
        {
            if(row==0){
               ...
               /*
                P2PDefenceAreaController *defenceArea = [[P2PDefenceAreaController alloc] init];
                defenceArea.contact = self.contact;
                [self presentViewController:defenceArea animated:YES completion:^{}];
                [defenceArea release];
                */
