        ...
        case 0:
        {
            if (self.bindIds.count<self.maxBindIdCount) {
                ...
                if(self.alarmState==SETTING_VALUE_ALARM_STATE_OFF&&sender.on){
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [self.tableView reloadData];
                        [self.view makeToast:[NSString stringWithFormat:@"%@ %li %@",NSLocalizedString(@"add_bind_account_prompt1", nil),(long)self.maxBindIdCount,NSLocalizedString(@"add_bind_account_prompt2", nil)]];
                    });
