...
typedef struct sAlarmInfoStore {
    DWORD dwTime_s; // 从 1970 年 1 月 1 日 00:00:00 开始计算的秒数, 报警发生的时间
    BYTE bAlarmType; // 报警类型
    BYTE bDefenceAndChannel; // 防区和通道(只有外部报警才有效), 前 4 个 bit 是防区号,后 4 个bit 是通道号
}PACKED sAlarmInfoStore;

typedef struct sMesgAlarmInfo{
    BYTE bCmd; // MESG_TYPE_GET_ALARM_INFO, //值:121
    BYTE bOption; // 0
    BYTE bAlarmInfoNs; // 0 报警记录的个数
    BYTE bReseve; // 0 保留位
    DWORD dwLastAlarmInfoIndex; // 最后一个报警值在记录报警记录以来的编号
    sAlarmInfoStore bAlarmInfo[150]; // 此项为空,只发送上面3个数据既能获取信息
}PACKED sMesgAlarmInfo;
