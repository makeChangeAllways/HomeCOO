...
+(NSString*)convertTimeByInterval:(NSString*)timeInterval{
    ...
    if (isDeviceAlarmRecord) {//修改默认时区
        [NSTimeZone setDefaultTimeZone:[NSTimeZone timeZoneWithAbbreviation:@"UTC"]];//UTC
    }
...
+(void)saveScreenshotFileWithId:(NSString*)contactId data:(NSData*)data{
    ...
    NSString *cur_contact = [user_default objectForKey:@"Current_Contact"];
    ...
    [data writeToFile:[NSString stringWithFormat:@"%@/%@%ld.png",savePath,cur_contact,timeInterval] atomically:YES];
}
...
{
    return  [NSTimer scheduledTimerWithTimeInterval:1 target:aTarget selector:aSelector userInfo:nil repeats:YES];
}
...
- (NSString *)getMd5_32Bit_String{//得到32位字符串
